...
```{r}
A_no_B <- setdiff(rownames(A),rownames(B))
A_no_B

B_no_A <- setdiff(rownames(B),rownames(A))
B_no_A

#Son los 7 animales, que son los siguientes artic fox, artic gruond squirrel, patas monkey, mole rat, dipliodocus, triceratops y bronchiosaurus
```

e.  Determina las diferencia entre los animales que no aparecen en ambos conjuntos.

```{r}
#Podemos observar que hay algunos animales que podemos intuir que si estan en los dos conjuntos, pero nos marcan que no estan porque estan mal escritos
```
...
#Podriamos decir que tinen casi el mismo comportaminto, porque en ambos graficos no muestran cambios determinantes
#Podemos decir que si es una distribucion gaussiana ya que todos los puntos estan comprendidos en la zona coloreada
```
...
3.  La función **library** sin argumentos abre una ventana y muestra las librerías que han sido instaladas.
    ...
    ```{r}
    lib <- library()
    head(lib$results,n=5)
    str(lib) #Podemos observar que hay 190 librerias instaladas
    ```
...
library(MASS)
library(car)
head(cabbages) #Cult, Data y HeadWt
head(Chile) #Region, Popular y sex

any(is.na(cabbages)) #En el paquete de cabbages no hay NA
any(is.na(Chile)) #En el paquete de Chile si que hay un Na
...
```{r}
qqPlot(cabbages$HeadWt)#En esta no cumple la distribucion Gaussiana porque podemos observar que algunos puntos estan fuera de la zona azul
qqPlot(cabbages$VitC)#En cambio en este casi si que cumple la funcion Guassiana
```
...
```{r}
mosaicplot(cabbages$VitC~cabbages$Cult, col=rainbow(2))

#Aunque no he he utilizado la funcion hist, he utilizado la funcion mosiacplot que tambien nos represtenta las dos variables.
```
...
#No encuentro el error del segundo plot
```
